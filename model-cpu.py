...
def replicator_clip_boost_cpu(
    ...
    """
    CPU-safe version:
      - forces float32 everywhere (no half)
      - avoids CUDA assumptions
    """
    device = torch.device("cpu")
    x_img = x_img.to(device=device, dtype=torch.float32)

    x = modelCLIP.visual.conv1(x_img)  # float32 on CPU
    fea_h, fea_w = x.shape[-2:]

    x = x.reshape(x.shape[0], x.shape[1], -1).permute(0, 2, 1)

    cls_tok = modelCLIP.visual.class_embedding.to(dtype=x.dtype, device=device)
    x = torch.cat([cls_tok + torch.zeros(x.size(0), 1, x.size(-1), device=device, dtype=x.dtype), x], 1)

    pos = modelCLIP.visual.positional_embedding.to(dtype=x.dtype, device=device)
    ...
    spatial_img_features = [t[1:, :] for t in x_normal]

    gt = torch.as_tensor(global_means_tensor, dtype=torch.float32, device=device)
    ...
    w = w_trans.view(x_img.shape[0], -1)

    if invert:
        w = 1.0 - w

    w_full = torch.cat([torch.zeros(x.size(1), 1, device=device, dtype=x.dtype), w.to(x.dtype)], dim=1)
    key_bias = w_full[:, None, None, :]

    # Branch B: boosted transformer
    x_boost = x_common
    for i in range(first_sel, len(blocks)):
        if i in sel:
...
def build_esquema_dict() -> Dict[str, Dict[str, List[List[float]]]]:
    ...
    def to_list_1d(x) -> List[float]:
        if isinstance(x, torch.Tensor):
            return x.detach().cpu().float().numpy().reshape(-1).tolist()
        import numpy as np
        return np.asarray(x, dtype=float).reshape(-1).tolist()

    def mean_embeddings(emb_list: Sequence) -> List[float]:
        import numpy as np
        arr = np.stack([to_list_1d(e) for e in emb_list], axis=0)
    ...
    for sp, emb in species_to_emb.items():
        order, family = taxonomy[sp]
        esquema_dict[sp] = {"group": f"Our_Name_{family}", "embedding": [to_list_1d(emb)]}

    for (order, family), sps in family_to_species.items():
        fam_embs = [species_to_emb[sp] for sp in sps]
        esquema_dict[family] = {"group": f"Family_{order}", "embedding": [mean_embeddings(fam_embs)]}

    for order, sps in order_to_species.items():
        ord_embs = [species_to_emb[sp] for sp in sps]
        esquema_dict[order] = {"group": f"Order_{order}", "embedding": [mean_embeddings(ord_embs)]}

    return esquema_dict


def normalize_encoded_dict_cpu_inplace() -> None:
    """
    CPU version:
      - ensure float32
...
def load_binary_efficientnet_cpu() -> nn.Module:
    """
    Load the 'Elasmobranch vs other' EfficientNet model on CPU.
    """
    device = torch.device("cpu")
...
def predict_folder_cpu(ruta: str) -> List[List[Optional[str]]]:
    """
    CPU-only end-to-end inference.
    """
    device = torch.device("cpu")
    ...
    # Load CLIP on CPU
    modelCLIP, preprocessCLIP = clip.load("ViT-L/14", "cpu")
    modelCLIP = modelCLIP.to(device).eval()
    ...
    # Binary model (elasmobranch vs other) on CPU
    ...
    modelEF = load_binary_efficientnet_cpu()

    with torch.no_grad():
        logits = modelEF(images_tensor_ef).squeeze(1)
        probs = torch.sigmoid(logits)
        preds = (probs >= 0.5).long()  # 1 => other, 0 => elasmobranch

    # CLIP embeddings on CPU
    images_tensor_clip = torch.stack([preprocessCLIP(img) for img in images]).to(device=device, dtype=torch.float32)

    clip_inres = modelCLIP.visual.input_resolution
    clip_ksize = modelCLIP.visual.conv1.kernel_size

    batch_size = 4  # CPU: usually smaller batch is safer
    ...
    with torch.no_grad():
        for i in range(0, images_tensor_clip.size(0), batch_size):
            ...
            img_emb = replicator_clip_boost_cpu(
                batch,
                alpha_w=1.4,
                invert=False,
                indices=None,
    ...
    normalize_encoded_dict_cpu_inplace()
...
# Backwards-compatible name (so you can keep calling model(ruta))
def main(ruta: str):
    print("CPU")
    return predict_folder_cpu(ruta)
